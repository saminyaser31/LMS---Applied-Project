...
class RouteServiceProvider extends ServiceProvider
{
    ...
    protected function configureRateLimiting(): void
    {
        ...
        RateLimiter::for('login', function (Request $request) {
            return Limit::perMinute(env('MAX_REQUEST_PER_HOUR', 15))->by($request->ip());
        });

        RateLimiter::for('forgetPassword', function (Request $request) {
            return Limit::perHour(env('MAX_REQUEST_PER_HOUR', 15))->by($request->ip());
        });
        ...
        RateLimiter::for('otpResend', function (Request $request) {
            return Limit::perHour(env('MAX_REQUEST_PER_HOUR', 15))->by($request->ip());
        });
